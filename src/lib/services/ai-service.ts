...
/**
 * 使用 Gemini AI 从文本中提取答案
 * @param text PDF 文本内容
 * @returns 提取的答案数组
 */
export async function extractAnswersWithAI(
  text: string,
  retryCount = 0
): Promise<any[]> {
  console.log(`调用 Gemini API 提取答案 (尝试 ${retryCount + 1}/4)`);
  ...
  try {
    ...
    // 准备提示词
    const prompt = `
      你是一个专业的环评师考试答案提取助手。请从以下文本中提取所有答案，并按照指定格式输出。

      请仔细分析文本，找出所有可能的答案。答案格式多样，可能包含如下形式：
      1. 常规答案：通常包含答案内容
      2. 图表答案：包含表格、图形或其他视觉元素的答案（在文本中可能表现为特殊格式或描述）

      【非常重要】: 这个文本仅仅是整个PDF的一部分，可能只包含部分答案或者只包含题目。请只提取你在这一段能看到的内容，不要担心答案是否完整或编号是否连续，我们会从多段文本中合并所有信息。
        答案分布有两种情况，一种是在题目后面，一种是在文本最后单独列出，答案与题目是通过题目最开始类似于2021-31，或者练习1等信息匹配的。需要你对此进行判断，并为每道题匹配答案。
        有的题的编号是2007改编（不定项），有的是2007（不定项）这是两道题，需要分别提取。
      # 提取规则
      对于每个答案，请提取以下可获得的信息：
      1. 答案编号（如"2022-7"、"练习1"等任何形式的编号）
      2. 答案内容
      3. 答案类型（单选题、多选题、不定项等）
      ...
      2. 不要在JSON前后添加附加文本
      3. 对于实在无法解析的内容，尝试提取尽可能多的有效信息
      4. 对于包含图表的答案，特别标注hasChart字段为true
      5. 保留答案的原始编号格式，不要统一转换
      6. 【重要】提取答案时，必须完整保留答案前面的标识符（如"1."、"二."等），以便与题目匹配
    ...
    // 使用自定义的 JSON 解析函数
    let answers = parseJsonWithChineseChars(content);
    ...
    if (!answers || !Array.isArray(answers) || answers.length === 0) {
      ...
      if (jsonMatch) {
        ...
        // 尝试解析提取的 JSON
        const extractedAnswers = parseJsonWithChineseChars(extractedJson);
        if (
          extractedAnswers &&
          Array.isArray(extractedAnswers) &&
          extractedAnswers.length > 0
        ) {
          console.log(`成功提取并解析了 ${extractedAnswers.length} 个答案`);
          console.log(
            "第一个答案示例:",
            JSON.stringify(extractedAnswers[0], null, 2)
          );
      ...
      if (!answers || !Array.isArray(answers)) {
        console.log("无法解析 API 响应为有效的 JSON 数组，返回空数组");
        return [];
      }
    }

    // 如果解析成功
    console.log(`提取了 ${answers.length} 个答案`);
    if (answers.length > 0) {
      console.log("第一个答案示例:", JSON.stringify(answers[0], null, 2));
    }
    ...
    // 如果还有重试次数，重试
    if (retryCount < maxRetries) {
      console.log(`重试提取答案 (${retryCount + 2}/${maxRetries + 1})`);
      return extractAnswersWithAI(text, retryCount + 1);
    }

    console.log("超过最大重试次数，返回空数组");
    return [];
  }
}
